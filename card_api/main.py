...
from paths import REPO_ROOT, runtime_json
...
# 載入資料（全局，一次載入）
try:
    with open(runtime_json('new_runes.json'), 'r', encoding='utf-8') as f:
        RUNES = json.load(f)
...
try:
    with open(runtime_json('runes_all_data.json'), 'r', encoding='utf-8') as f:
...
# 載入三卡組合表
try:
    with open(runtime_json('three_card_combinations.json'), 'r', encoding='utf-8') as f:
...
# LOC7 FAQ 資料（全局，一次載入）
FAQ_DATA_PATH = runtime_json("LOC_FAQ_RAG_v0.4.json")
try:
...
LOC3_DATA_PATH = runtime_json("LOC3_LYRICS_SEARCH_v0.1.json")
try:
