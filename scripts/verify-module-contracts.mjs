...
const runesClient=readFileSync(resolve(root,'app/runes/RuneDrawClient.jsx'),'utf8');
for(const token of ['LOC_DATA.RUNES','LOC_DATA.LOTS','data-draw-action="execute"','function executeDraw','function finishDraw'])if(!runesClient.includes(token))failures.push(`RuneDrawClient: missing draw contract ${token}`);
...
const coreBatch=/fetchLocJsonBatch\(\[LOC_DATA\.RUNES,\s*LOC_DATA\.LOTS,\s*LOC_DATA\.RUNE_INTERPRETATIONS\]/.test(runesClient);
...
if(!(coreBatch||(directRunes&&directLots)))failures.push('RuneDrawClient: canonical RUNES/LOTS must load through the local-first Next data loader');
