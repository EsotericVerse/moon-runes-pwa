...
for(const [id,domain] of Object.entries(expectedDomains)){
  ...
  for(const feature of FEATURES_V2)if(featureHrefV2(id,feature.id)!==`https://${domain}/${feature.path}`)throw new Error(`${id}/${feature.id} route drifted`);
}
...
console.log('V2 registry-driven single NAV verified.');
