...
for(const token of ['lrunes.lo3rwang.cc','lo3rwang.lo3rwang.cc','admin.lo3rwang.cc','月之符文','語彙','風格詞','治理規則','脈絡','統計','文化','治理','context','statics','evolution','governance','search']){
  if(!nav.includes(token)){console.error('Missing scoped NAV contract: '+token);process.exit(1);}
}
for(const token of ['LOC Scope：月之符文','LunaRunes Scope：語彙','Author Scope：風格詞','Governance Scope：治理規則','admin.lo3rwang.cc']){
