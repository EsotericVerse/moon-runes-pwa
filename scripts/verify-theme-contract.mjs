...
if(!scopeSettings.includes('silver.loc_scope')||!scopeSettings.includes('default_theme_id'))failures.push('scope theme must live in silver.loc_scope');
if(scopeSettings.includes('api.scope_theme_defaults'))failures.push('retired scope theme table remains in the app');
if(!scopeSettings.includes("scope==='moon-runes'?'runes'"))failures.push('legacy rune scope must map to canonical runes');
...
if(!neonRepository.includes("'silver.loc_scope'"))failures.push('Neon repository must permit authorized scope theme updates');
...
console.log('[theme-contract] eight scalar themes and silver.loc_scope defaults verified');
