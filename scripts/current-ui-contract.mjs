...
const footer=fs.readFileSync('app/GlobalFooter.jsx','utf8');
...
const requiredNav=['月之符文','脈絡','統計','文化','治理','/governance','設定','抽牌','符文圖鑑','遊戲','符文脈絡','符文統計','符文文化','抽籤紀錄','搜尋','回月典首頁'];
const requiredFooter=['Lucas Oscar Wang 政德','聯絡方式'];
const forbiddenFooter=['秘藝文域（EsotericVerse）（籌備中）'];
...
const missing=[...requiredHome.filter(x=>!home.includes(x)),...requiredNav.filter(x=>!nav.includes(x)),...requiredFooter.filter(x=>!footer.includes(x)),...requiredRuneCanon.filter(x=>!runeCanon.includes(x)),...requiredArchitectureCanon.filter(x=>!architectureCanon.includes(x)),...requiredGovernance.filter(x=>!governancePage.includes(x))];
const forbidden=[...forbiddenHome.filter(x=>home.includes(x)),...forbiddenFooter.filter(x=>footer.includes(x))];
if(missing.length||forbidden.length){
  if(missing.length) console.error('Missing current UI contract: '+missing.join(', '));
  if(forbidden.length) console.error('Forbidden legacy UI contract: '+forbidden.join(', '));
  process.exit(1);
}
