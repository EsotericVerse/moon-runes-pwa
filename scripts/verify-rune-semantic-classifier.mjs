...
const cases=[
  ['時空',['時','空']],['風向',['風','向']],['明火',['明','火']],['天時',['時','緣']],
  ['時辰',['辰']],['清明',['辰']],['日期',['時']],['日蝕',['日']],['月蝕',['月']],
  ['日月當空',['明']],['延誤',['緣']],['誤判',['誤']],['誤解',['誤']],['錯誤資料',['誤']]
];
for(const [text,expected] of cases){
  const result=classifyRuneSemantics(text);
  assert.deepEqual(result.runes,expected,`${text}: ${JSON.stringify(result)}`);
...
console.log(`Rune semantic classifier verified: ${cases.length+4} governed cases.`);
