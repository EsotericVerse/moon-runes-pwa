...
每個介面只有一條正式導覽列（NAV）。頁內目錄、本地功能入口、返回連結、架構圖與快捷選單可以存在，但不是另一套 NAV；Current 文件與程式不得再使用 NAV1／NAV2／NAV3 名稱。
...
頁內功能入口與快捷選單不是 NAV。LunaRunes 首頁目前允許「抽牌｜符文圖鑑」頁內子選單；這不建立第二條正式 NAV，也不改變 Scope、資料歸屬或管理權。
