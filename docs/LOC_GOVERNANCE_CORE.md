...
符文治理／Grammar 以 `LunaRune64.xlsx#Grammar` 為最高優先結構化來源。

- 全域層次：詞彙層 → 關係層 → 位置層 → 場域層 → 時間修飾層；後層不得覆蓋符文本義。
- 單卡：符文本義＋方向＋卡片固有月相；真實月相只作低權重時間修飾。
- 兩卡：卡 A＋卡 B＋兩卡方向關係＋交互或治理原則。因果可以由牌位／問題建立，但符文交互仍用於描述共鳴、互補、衝突與語氣張力。
- 三卡：源＋轉＋合＋交互或治理原則。
- 五卡：過去＋現在＋未來＋環境＋心境＋交互或治理原則；不是 2+3 拼接，只連具有語義關係的卡對。
- 五卡關係優先：因→現、現→向、境→現／向、心→現／向，必要時因→向。
- OW3gs：1–6＝因的描述層；7–11＝果的判定層。先讀 7–11，再掃描 1–6，只回填明確語義關係，最後套月相交互。
- 1–6 可做群組、重複主題與張力聚合；不採「群組交替」作為現行 Grammar。
- 符文交互只作關係修飾，不單獨決定吉凶。
- 符文沙盒 Graph 是另一場域，可使用平等節點互動；不得直接覆蓋一般占卜牌陣 Grammar。
