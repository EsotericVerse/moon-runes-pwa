...
- Runtime: Next server routes → direct Neon SELECT / controlled link writes
...
The Next application runs as an OpenNext server on Cloudflare. Neon-backed routes require the deployment DATABASE_URL secret; the browser never receives that credential.
...
1. the Next server build succeeds;
2. every registered runtime source path resolves to an explicit Neon canonical route;
