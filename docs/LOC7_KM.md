...
月之符文是 LOC 的基本語意參照；LOC 是承接、分類、關聯、搜尋與演化這些語意結果的框架。符文治理／Grammar 的最高優先結構化來源為 `LunaRune64.xlsx#Grammar`；衍生文件、FAQ、RAG 與 Search 不得自行改寫其規則。

### 7.1 全域語法分層

```text
詞彙層 → 關係層 → 位置層 → 場域層 → 時間修飾層
```
...
```text
卡 A＋卡 B＋兩卡方向關係＋交互或治理原則
```

- 一般占卜可依位置／問題套用因果治理，例如第 1 張作因、第 2 張作果。
- 符文交互仍保留，用來描述兩個語義主題的共鳴、互補、衝突與語氣張力。
- 交互只作關係修飾，不改寫符文本義，也不單獨決定吉凶。
- 不採「群組交替」作為兩卡預設理論。
...
```text
1–6 因的描述層 ＋ 7–11 果的判定層 ＋ 交互或治理原則
```

十一張牌不是等權並列。

- 1–6：背景、既有條件、資源、阻力、外部擾動、尚未成形因素。
- 1–6 可用群組、重複主題與張力聚合，不要求逐張成句。
- 7–11：7 因、8 現、9 向、10 境、11 心，是核心判定區間。
- 讀取順序：先讀 7–11 → 再掃描 1–6 → 只回填有明確語義關係的場域訊息 → 最後套月相交互。
- 「群組、重複主題與張力聚合」不等於「群組交替」；後者不列入現行 Grammar。

### 7.7 符文交互與月相交互

**符文交互**：描述兩個語義主題的共鳴、互補、衝突與語氣張力，只作關係修飾，不改寫符文本義。

**月相交互**：卡片月相與真實月相的交互作用；真實月相是額外、小幅、低權重的時間情境修飾。

### 7.8 符文治理的正向與反向

1. **Forward：符文 → 現況解讀**
   - 依符文本義、方向、位置、交互／治理原則與問題脈絡解讀。
...
符文沙盒與一般占卜 Grammar 是不同場域。沙盒中的 Graph 可採平等節點互動、合作與交替，形成暫時定義；一般占卜則以各牌陣的位置與治理原則為主。Search／RAG 必須先辨識場域，不得把沙盒 Graph 規則直接套到一般解牌。
