# LOC Canon 1.2

**Status:** Current Canon  
**Date:** 2026-09-16
**Supersedes:** LOC Canon 1.1 for Current terminology and governance. Older Canon remains Historical and must not be rewritten.

## 1. Identity and purpose

LOC（月典）是可公開、可移植的語言模組框架（Modelized Language Framework）。LunaRunes（月之符文）是具有自己 Master Data、Canon 與治理權威的符號式語言（Symbolic Language），也是參考實作；它不等於 LOC 全域。

Canon 定義原則、權威、責任與邊界，不指定頁面框架、部署平台、主機、資料庫、快取或其他 implementation。LOC 不依賴 WordPress，也不綁定 Next.js、Vercel、Render、Supabase、KV 或任何特定供應商。

## 2. Scope and authority

治理判定依序確認：

**資料範圍（Scope）→ 權威來源 → Current 狀態 → 衝突處理**

Master Data、Canon、Spec、Registry、來源紀錄與衍生 View 只在各自責任範圍內生效。衍生資料不得反向覆寫上游權威；權威不足或衝突不能判定時，標記待治理，不猜測。

Scope 可新增、刪除、拆分、合併、重新命名或調整關係，不存在永久唯一的 Scope 清單。個人、符文、治理、作品、群組、部門與組織都可以形成獨立或多層 Scope。

每個 LOC instance 的管理權獨立。採用 LOC、讀取公開資料或引用 Canon，不代表 LOC 作者、原專案管理者或其他 instance 可以登入或管理該 instance。

## 3. Current and Historical

Current 只使用目前有效的名稱、定義、規則與資料狀態。歷史紀錄不可改寫；錯誤以更正、修訂或 supersede 新增紀錄，不靜默覆蓋原紀錄。

相應 Scope 的管理者可以治理哪些歷史資料被 Current 採用、分析或公開呈現，並必須留下稽核紀錄。已廢止的 **LOC1–8** 只可存在於明確標示的 Historical 紀錄，不得作為 Current 架構、模組名、欄位、導覽或責任識別。

## 4. Shared governance principles

- 客觀與中立：分析依資料、語境與公開規則，不迎合作者、管理者或預期答案。
- 可解釋：保留規則版本、證據、候選、排除與爭議狀態。
- 治理先於實作：先確定名稱、語意、權威與邊界，再更新程式、介面、索引、搜尋與推演。
- 語意優先：不以單字命中代替詞性／句內角色、主體性及所屬語意的判定。
- 分類一致：需要唯一值時輸出一個 Current 結果；接近候選另標 disputed，不以多值逃避判定。
- 資料歸屬：統合、引用與可讀取不改變來源 Scope 的所有權或寫入權。
- 寫入審核：跨 Scope 寫入或納入統合 Scope 必須由目標 Scope 審核。
- 可移植（Portable）：原則與資料契約不依附特定技術或平台。
- 多層治理：每層 Scope 保有自己的管理、資料與權威。
- 角色分離：作者、資料主體、Scope 管理者與系統管理者不得混同。
- 衝突不猜測：不能證明時保留未知、證據與待治理狀態。
- **實例門檻（Example-Gated Update）**：任何新的系統論點、治理原則、語法規則或模組定義，至少必須提出一個可檢查、可對照的具體實例，才可升級為 Current Canon。沒有實例時只能保留為 hypothesis／draft，不得只因概念聽起來合理而更新 Canon。

實例的功能不是替論點製造權威，而是讓抽象主張可以被說明、檢查、比較與修正。若實例與論點不符，應修正論點；不得反向改寫實例以配合 Canon。

## 5. Search, statistics and integration

個人、LunaRunes、治理與其他 Scope 各自保有資料歸屬、搜尋、統計與排行榜。月典結果是經審核的統合層，不抹除來源，也不使來源 Scope 永久互通。

資料首次進行完整分析；同筆資料未變則跳過，有差異才更新，修正標記只觸發相關局部重算。排行榜保存 Current count 並進行差異更新，避免為每次呈現反覆全文搜尋。

## 6. ERA governance

時期（ERA）屬於各自 Scope，可由該 Scope 治理修改。個人 ERA、LunaRunes ERA、治理 ERA 與其他組織 ERA 不得因使用同一框架而混成單一時間線。跨 Scope 比較必須保留各自識別與來源。

## 7. Navigation

LOC 只有一條正式導覽列（NAV）。架構圖、本地功能入口、頁內目錄與快捷選單不是 NAV2 或 NAV3，也不得以這些名稱重新建立多套導覽治理。

導覽項目的顯示文字可由其所屬 Scope 治理；例如個人首頁第一個 NAV 文字可由個人 Scope 管理者修改。這類設定只在該 Scope／instance 生效，不改寫 LOC 全域 Canon。

## 8. LunaRunes boundary

LunaRunes Scope 由其 Master Data／Base66、Current Canon／Spec 與 Registry 治理。符文分類依詞性／句內語意角色、群組主體性、符文語意歸屬逐層判定。關鍵詞是證據，不是直接命中規則。

LunaRunes 的 Current／Historical、ERA、搜尋、統計與排行榜保持獨立，不混入政德個人文章或個人 ERA。LOC 全域原則約束共同邊界，但不取代 LunaRunes 的權威來源。

### 8.1 最小語意種子與組合式模型

LunaRunes 作為符號式語言，不以「特立獨行」或反流行作為價值，也不以當下流行的道德判斷作為預設答案。作者方法的出發點是：**無權替其他人預先決定是非，因此系統先提供最基本的語意種子、關係與條件，保留脈絡，讓使用者依自己的情境判斷。**

月之符文的核心不是把每一個特殊情況都製造成新規則，而是以有限、可辨識的基本特質作為種子，再由交互與組合描述複雜現象：

- **單卡**：基本特質種子／最小語意單位。
- **雙卡**：最小交互模型，描述兩個特質之間的關係、因果或脈絡；不內建好壞與是非判決。
- **三卡**：`源－轉－合`。在關係之上加入一個變數／轉折，配合時間基準點、階段與作用範圍推論可能性；不是宣告「最佳時機」或固定未來。
- **五卡**：由雙卡的關係／因果與三卡的源轉合進一步組合，形成有限複雜度的文化／情境描述；變數保持可辨識，避免為了複雜而失去可解釋性。
- **OW3gs 11 卡**：作者特化的三維震盪模板，把多組關係、時期與特質放入同一情景中整合。OW3gs 的 11 卡位置屬作者保留實作；其他使用者仍可在其他牌數／位置建立自己的自訂 Grammar 與註解，只要清楚標示其 Scope 與來源。

高階模型必須可以回溯到底層單卡特質與雙卡交互；複雜度來自組合，而不是以更多不可解釋的特殊規則取代基本單位。

**治理句：沒有預設的是非，只有忠實的紀錄與可追溯的組合。**

### 8.2 關係、時期與整合特質

上述組合可作為 LunaRunes 特質化模板的代表實例：

`單卡（Seed） → 雙卡（Relation / Context） → 三卡（Period + Context / 源轉合） → 五卡（Culture / 有限複雜情境） → OW3gs 11 卡（三維震盪模板／完整情景）`

這條鏈的目的不是宣稱張數本身代表宇宙層級，而是展示同一批基本語意種子如何逐層增加分析條件。

在進階分析中，**時、辰、緣必須保持獨立**：時提供時間基準／時序；辰提供 Phase／階段與位置；緣提供關係形成與交會條件。三者可共同參與推演，但不得互相替代。

## 9. OW3gs provenance and naming governance

OW3gs 是簡稱／別名；**最早正式定義的全名為 `Oscillation World 3 Grounds System`**。它形成於作者採用「王政德／政德」這一 Current 名稱之前，也早於後來的 `lo3rwang` 公開識別，因此不得以後來身份替它補造前綴或改寫名稱來源。

Current 命名治理如下：

- 歷史正式名稱：**Oscillation World 3 Grounds System**。
- 歷史簡稱／別名：**OW3gs**。
- 作者自己的中文別名／說明：**三維震盪模板**。
- Current 語意上優先稱為「**模板**」，因為它描述的是可套用、可組合、可分析的三維震盪結構；不必把它重新升格成獨立框架或新系統名稱。
- `OW3gs` 不得回溯改名或反向解釋為 `LOW3GS`、`LOW 3 Grounds System` 或其他以 `LOW` 為前綴的名稱。
- `lo3rwang` 是後來形成的公開作者識別；它不是 `LOW` 的縮寫。作者刻意不以 `LOW` 作為統一縮寫，避免與英文 `low`（低）產生不需要的字義聯想。
- OW3gs 現行歸屬 **德之符文（De／Rune 0）** 的作者特化方法／治理脈絡，但此 Current 歸屬不得改寫其早於「政德」名稱的歷史。
- 今日可用「三個分析基底／三個維度」說明 OW3gs 的方法結構：**一維關係、二維時期、三維整合特質／完整情景**。這是對 `Oscillation World 3 Grounds System` 的 Current 方法整理，不取代其歷史正式全名。

### 9.1 OW3gs 與 LOC 的前因後果

歷史／方法論方向為：

`Oscillation World 3 Grounds System／OW3gs 的實際分析與組合經驗 → 方法整理、抽象與模組化 → LOC／月典`

因此不是「LOC 先建立，再發明 OW3gs」。LOC 是在整理既有方法時逐漸形成的更上層語言模組框架，用來處理不同模組的分析、分類、關係、治理、搜尋、歷史與演化。

概念上，可以把 LOC 視為在 OW3gs 三個分析基底之外，再增加「對分析方法本身進行治理、模組化與演化整理」的一層；但 **`4gs` 不是正式名稱，也不需要建立為新模組**。這個四維描述只用來說明 LOC 如何由實作經驗向上抽象，不取代 LOC 正式名稱與責任。

這段歷史本身也是「實例門檻」的代表：LOC 的模組化原則不是只從抽象論點產生，而有 LunaRunes／OW3gs 的實際組合模板作為可檢查案例。

## 10. Governance execution and audit

治理空間包含公開原則、具權限的管理者功能與治理紀錄。可執行設定至少涵蓋 Scope 與關係、資料納入／移除、審核、ERA、修正標記、統合、搜尋／統計／排行榜維護及角色權限。

每筆治理變更至少記錄治理對象、Scope、操作者角色、動作、前後值、理由、證據、時間、審核狀態與替代紀錄。

## 11. Copyleft

LOC 的基本方法論依 Copyleft 原則開放使用與研究，使用及衍生時須保留必要來源、作者、歷史與修改標示。衍生商業使用須取得同意；解析介面、推演層及其他受管理服務可以收費。

此段是 Current 治理意圖，不自動等同或改寫為 GPL、AGPL、CC BY-SA 或其他既有 license。第三方內容、私人資料及另有權利限制的資產不因本原則自動重新授權。
