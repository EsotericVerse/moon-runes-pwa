...
只有「資料量較大、來源權威明確、結構成熟、runtime 需要穩定讀取，而且不應由下游任意手改」的資料才進 `core/`。

ERA、Governance、Rights、Graph Schema、KM Registry 等小型但會持續治理更新的 JSON，仍放在 `registries/`，透過版本與治理流程持續變動。
