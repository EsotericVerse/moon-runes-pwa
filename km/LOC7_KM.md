...
Canonical Graph RAG 已進入現行 Unified Search：Search retrieval 先選 seed，再以受治理的 edge 做 bounded 1–3 hop traversal，並把 ERA、LOC、Work、Media、Knowledge 與 LOC8 temporal observation 聚合到 Search Synthesis。Graph Schema 現行為 v0.5。
...
現行預設：`min_traversal_score=0.25`、`hop_decay=0.88`。權重與 band 的唯一機器可讀 contract 為 `data/json/registries/LOC_GRAPH_SCHEMA.json` v0.5。
