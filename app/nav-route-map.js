...
export function detectNavScope(pathname='/',host=''){
  ...
  if(host==='lo3rwang.lo3rwang.cc')return 'lo3rwang';
...
export function getNavScopeConfig(scope,host=''){
  if(scope==='runes'){
    ...
    return {base,reserved:['語彙',base||'/'],role:[['作者頁面','https://lo3rwang.lo3rwang.cc']],homes:[['回月之符文首頁',base||'/'],['回月典首頁','https://loc.lo3rwang.cc']]};
  }
  if(scope==='lo3rwang')return {base:'',reserved:['風格詞','/'],role:[['管理者頁面','https://manage.lo3rwang.cc']],homes:[['回作者頁面','/'],['回月典首頁','https://loc.lo3rwang.cc']]};
  ...
  return {base:'',reserved:['月之符文','/runes'],role:[['作者頁面','https://lo3rwang.lo3rwang.cc']],homes:[['回月典首頁','/']]};
}
