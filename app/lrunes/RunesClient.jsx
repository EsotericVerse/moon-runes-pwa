...
export default function RunesClient(){
  ...
  useEffect(()=>{setModeKey(initialMode());setActiveSection(initialSection());let live=true;if(account.user)listNeonRecords('rune-draw').then(rows=>{if(live)setDrawRecords(sortRecords(rows));}).catch(()=>{if(live)setRecordStatus('Neon 抽籤紀錄讀取失敗。');});else setDrawRecords([]);fetchNeonDataBatch([LOC_DATA.RUNES,LOC_DATA.LOTS,LOC_DATA.RUNE_INTERPRETATIONS],{concurrency:2}).then(([runes,lots,interpretationRows])=>{if(!live)return;const canonicalRunes=(runes||[]).filter(row=>Number(row?.編號)>=1&&Number(row?.編號)<=66);if(canonicalRunes.length<66)throw new Error(`核心符文資料只有 ${canonicalRunes.length} 枚，無法安全抽牌。`);setData({runes:canonicalRunes,lots:Array.isArray(lots)?lots:[],history:{},writing:{},eras:{}});setInterpretations(Array.isArray(interpretationRows)?interpretationRows:[]);setError('');fetchNeonDataBatch([LOC_DATA.LRUNES_PERIODS,LOC_DATA.LO3RWANG_PERIODS],{concurrency:2}).then(([history,eras])=>{if(!live)return;setData(previous=>previous?{...previous,history:history||{},eras:eras||{}}:previous);}).catch(()=>{});}).catch(err=>live&&setError(`月之符文核心資料載入失敗：${err?.message||'未知錯誤'}`));return()=>{live=false;timers.current.forEach(clearTimeout);};},[account.user?.id]);
