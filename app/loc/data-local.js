...
export async function getLocalDataSegment(path){if(!browserReady())return null;const db=await openDb();const tx=db.transaction(SEGMENT_STORE,'readonly');const value=await requestToPromise(tx.objectStore(SEGMENT_STORE).get(path));await transactionDone(tx);return value;}
export async function getFreshLocalDataSegment(path,{hash='',version=''}={}){const value=await getLocalDataSegment(path);if(!value)return null;if(hash)return value.hash===hash?value:null;if(version)return value.source_version===version?value:null;return value;}
...
export async function deleteLocalDataSegment(path){if(!browserReady())return;const db=await openDb();const tx=db.transaction(SEGMENT_STORE,'readwrite');tx.objectStore(SEGMENT_STORE).delete(path);await transactionDone(tx);}
