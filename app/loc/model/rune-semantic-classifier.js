...
const GROUP_BY_RUNE = Object.freeze({
  ...
  日:'定序',月:'定序',星:'定序',辰:'定序',明:'定序',時:'定序',空:'定序',因:'定序',
  福:'無序',禍:'無序',無:'無序',夢:'無序',幻:'無序',緣:'無序',虛:'無序',果:'無序',
  德:'系統特別',玄:'系統特別',命:'系統特別'
});
...
const AUTHOR_RULES = Object.freeze([
  ['微月光','德'],['人生月台','德'],['斜教','德'],['OW3gs','德']
]);
...
function occupiedByLongerRule(text, index, rune, matchedRules){
  return matchedRules.some(rule=>rule.pattern.length>1 && rule.pattern.includes(rune) && index>=rule.index && index<rule.index+rule.pattern.length);
}
...
export function classifyRuneSemantics(input,{authorScope=false}={}){
  ...
  // Longest complete expressions win locally before character-level candidates.
  for(const rule of [...RULES].sort((a,b)=>b.pattern.length-a.pattern.length)){
    let from=0;
    while(from<text.length){
      const index=text.indexOf(rule.pattern,from);
      if(index<0) break;
      const overlaps=occupied.some(([start,end])=>index<end && index+rule.pattern.length>start);
      if(!overlaps){
        matchedRules.push({...rule,index});
        occupied.push([index,index+rule.pattern.length]);
      }
  ...
  if(authorScope){
    for(const [pattern,rune] of AUTHOR_RULES){
      let from=0;
      while(from<text.length){
        const index=text.indexOf(pattern,from);
        if(index<0) break;
        matchedRules.push({id:`author-${pattern}`,operation:'OVERRIDE',pattern,runes:[rune],reason:'Author Governance 保留詞。',index});
        from=index+pattern.length;
      }
    }
  }

  const semanticHits=matchedRules.flatMap(rule=>rule.runes.map(rune=>({
    rune,group:GROUP_BY_RUNE[rune],operation:rule.operation,pattern:rule.pattern,index:rule.index,reason:rule.reason,rule:rule.id
  })));

  const deferred=[];
  const literalCandidates=[];
  for(let index=0;index<text.length;index++){
    const rune=text[index];
    if(!GROUP_BY_RUNE[rune]) continue;
    if(occupiedByLongerRule(text,index,rune,matchedRules)) continue;
    if(DEFERRED_LITERAL_RUNES.has(rune)){
      deferred.push({rune,index,reason:`${rune} 為 DEFER／Scope 管理字面候選，不能只憑字元命中。`});
      continue;
    }
    literalCandidates.push({rune,index,group:GROUP_BY_RUNE[rune],reason:'字面只建立候選；需要完整上下文確認實際語意。'});
  }

  const runes=unique(semanticHits.map(hit=>hit.rune));
  return {
    text,
    runes,
    groups:groupsFor(runes),
    semantic_hits:semanticHits,
    candidates:literalCandidates,
    deferred,
    disputed:deferred.length>0 || literalCandidates.length>0,
    ruleset_version:RUNE_SEMANTIC_RULESET,
    api_used:false
  };
}

export function getRuneSemanticRules(){
  return {rules:RULES,authorRules:AUTHOR_RULES,deferredLiteralRunes:[...DEFERRED_LITERAL_RUNES]};
}
