...
function unique(values){return [...new Set(values)]}function groupsFor(runes){return unique(runes.map(r=>GROUP_BY_RUNE[r]).filter(Boolean))}
...
export function classifyRuneSemantics(input,{authorScope=false}={}){const text=String(input??'');const matchedRules=[];const occupied=[];for(const rule of [...RULES].sort((a,b)=>b.pattern.length-a.pattern.length)){let from=0;while(from<text.length){const index=text.indexOf(rule.pattern,from);if(index<0)break;const overlaps=occupied.some(([start,end])=>index<end&&index+rule.pattern.length>start);if(!overlaps){matchedRules.push({...rule,index});occupied.push([index,index+rule.pattern.length])}from=index+rule.pattern.length}}if(authorScope){for(const [pattern,rune] of AUTHOR_RULES){let from=0;while(from<text.length){const index=text.indexOf(pattern,from);if(index<0)break;matchedRules.push({id:`author-${pattern}`,operation:'OVERRIDE',pattern,runes:[rune],reason:'Author Governance 保留詞。',index});from=index+pattern.length}}}const semanticHits=matchedRules.flatMap(rule=>rule.runes.map(rune=>({rune,group:GROUP_BY_RUNE[rune],operation:rule.operation,pattern:rule.pattern,index:rule.index,reason:rule.reason,rule:rule.id})));const deferred=[];const literalCandidates=[];for(let index=0;index<text.length;index++){const rune=text[index];if(!GROUP_BY_RUNE[rune])continue;if(occupiedByLongerRule(index,rune,matchedRules))continue;if(DEFERRED_LITERAL_RUNES.has(rune)){deferred.push({rune,index,reason:`${rune} 為 DEFER／Scope 管理字面候選，不能只憑字元命中。`});continue}literalCandidates.push({rune,index,group:GROUP_BY_RUNE[rune],reason:'字面只建立候選；需要完整上下文確認實際語意。'})}const runes=unique(semanticHits.map(hit=>hit.rune));return{text,runes,groups:groupsFor(runes),semantic_hits:semanticHits,candidates:literalCandidates,deferred,disputed:deferred.length>0||literalCandidates.length>0,ruleset_version:RUNE_SEMANTIC_RULESET,api_used:false}}
