...
const PRINCIPLES = [
  ['客觀與中立（Objectivity and Neutrality）','依資料、語境與公開規則判定；分析結果與 lo3rwang、管理者或使用者的價值判斷分開。'],
  ...
  ['同原則、不同主體','LOC 提供共同治理方法與分類框架；LunaRunes、lo3rwang 與其他 Scope 可以共用相同原則，但各自管理不同主體、資料與權威。'],
  ['漸進式揭露（Progressive Disclosure）','使用者不需要先理解整套 LOC、所有 LunaRunes 或 lo3rwang 全部作品。可從有興趣的 Feature 進入，再依需要逐步展開底層結構與治理資訊。'],
  ...
  ['多層治理與獨立管理','Scope 可依個人、群組、部門或組織新增、刪除、拆分、合併與調整，不存在永久唯一的 Scope 清單。每個 LOC instance 的管理權獨立；採用 LOC 不授予 lo3rwang 登入權。'],
  ...
  ['時期獨立','時期屬於各自分頁／功能，可由其管理功能治理修改；個人、符文、治理或組織的時期不得混成一條時間線。'],
  ['角色分離','創作者、資料主體、Scope 管理者與系統管理者是不同角色；管理資料不等於取得 lo3rwang 身分、觀點代表權或其他 instance 的權限。'],
...
const SECTIONS = [
  ...
  },
  {
    ...
    content:<>
      <p>LOC 以 Copyleft 為根本方向：基本方法論可供研究、理解與延伸，但必須保留必要來源、創作者與修改標示；衍生商業使用與專業實作依個案另行治理。</p>
      <p>採用 LOC 不代表取得 lo3rwang 身分、其他 Scope 的資料所有權、寫入權或治理權；授權與 Scope Authority 是不同層次。</p>
    </>,
    links:[
...
export default function GovernanceView(){
  return <PageComposition
    ...
    intro={<>
      <p>LOC Governance 是整個系統的治理入口：總覽 Current 原則、Scope 關係、權威邊界、授權與歷史，但不取代 LunaRunes、lo3rwang 或其他 Scope 對自身資料的治理。</p>
