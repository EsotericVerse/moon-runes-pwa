...
export default function LibraryView(){
    ...
    try{
      ...
      for(const raw of rows){
        const record=raw?.type===LIBRARY_RECORD_TYPE&&raw?.id
          ...
          :createLibraryRecord({title:raw?.title,text:raw?.text??raw?.content,source:raw?.source||`import:${file.name}`,classification:raw?.classification||null});
  ...
  return <section className="loc-view">
    <header className="loc-hero">
      ...
      <h1>Library</h1>
      <p>原始文字與分類結果都存在瀏覽器 IndexedDB。修改群組規則後，可重新分類而不改寫原始文字。</p>
    </header>

    <section className="loc-card">
      <div className="loc-actions">
        <a className="loc-button primary" href="/classify">＋新增／分類文字</a>
        <button className="loc-button" onClick={()=>downloadJsonFile({version:1,records},'loc-library.json')} disabled={!records.length}>匯出 JSON</button>
