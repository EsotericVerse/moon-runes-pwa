...
export default function ContextView(){
  ...
  return <section className="loc-view">
    ...
    {tab==='events'&&<section className="loc-card"><p className="loc-eyebrow">Context / Event · 脈絡／事件</p><h2>事件</h2>{!events?<p>載入事件…</p>:<><div className="loc-context-list">{eventRows.slice((eventPage-1)*PAGE_SIZE,eventPage*PAGE_SIZE).map(item=><article className="loc-context-item" key={item.id}><div className="loc-result-meta"><time>{item.date}</time><span>{item.event_type||item.status}</span></div><h3>{item.title}</h3><p>{item.description}</p>{item.state_after&&<p><strong>狀態 State →</strong> {item.state_after}</p>}</article>)}</div><Pagination page={eventPage} total={eventRows.length} onChange={setEventPage}/></>}</section>}

    {tab==='relations'&&<section className="loc-card"><p className="loc-eyebrow">Context / Relation · 脈絡／關係</p><h2>關係式</h2>{!relations?<p>載入關係…</p>:<div className="loc-context-list">{relationRows.map(item=><article className="loc-context-item" key={item.relationship_id}><div className="loc-result-meta"><span>類型 Type：{item.relation_type}</span><span>證據 Evidence：{item.evidence_status}</span></div><h3>{item.source?.title||item.canonical_key}</h3><p>{item.relation_summary}</p><div className="loc-chip-list">{item.targets?.map(target=><span key={`${item.relationship_id}-${target.work_ref}`}>→ {target.title||target.work_ref}</span>)}</div></article>)}</div>}</section>}

    {tab==='scenarios'&&<section className="loc-card"><p className="loc-eyebrow">Context / Scenario · 脈絡／情境</p><h2>情境語料庫 Scenario Corpus</h2>{!scenarios?<p>載入情境 Scenario…</p>:<><div className="loc-filter-row"><select value={scenarioGroup} onChange={e=>{setScenarioGroup(e.target.value);setScenarioPage(1)}}><option value="">全部群組</option>{groups.map(group=><option key={group} value={group}>{group}</option>)}</select><input value={scenarioQuery} onChange={e=>{setScenarioQuery(e.target.value);setScenarioPage(1)}} placeholder="搜尋情境"/></div><div className="loc-context-list">{filteredScenarios.slice((scenarioPage-1)*PAGE_SIZE,scenarioPage*PAGE_SIZE).map(item=><article className="loc-context-item" key={item.event_id}><div className="loc-result-meta"><span>群組 Group：{item.event_group}</span><span>需求簽章 Signature：{item.requirement_signature}</span></div><h3>{item.event_id} · {item.title}</h3><p>{item.description}</p></article>)}</div><Pagination page={scenarioPage} total={filteredScenarios.length} onChange={setScenarioPage}/></>}</section>}

    {tab==='graph'&&<section className="loc-card"><p className="loc-eyebrow">LunaRunes Semantic Graph · 月之符文語意圖 · No API</p><h2>符文語意圖 Semantic Graph</h2>{!runes?<p>載入正式符文資料 canonical runes.json…</p>:<><p>直接由 runes.json 的群組、正向關鍵詞與反向關鍵詞即時計算；不保存第二份節點資料庫（node database）。</p><div className="loc-filter-row"><select value={graphGroup} onChange={e=>setGraphGroup(e.target.value)}><option value="">全部群組</option>{runeGroups.map(group=><option key={group} value={group}>{group}</option>)}</select><input value={graphQuery} onChange={e=>setGraphQuery(e.target.value)} placeholder="搜尋符文、群組或關鍵詞"/></div><div className="loc-metrics"><div><small>可見節點 Nodes</small><strong>{filteredNodes.length}</strong></div><div><small>可見關係 Edges</small><strong>{filteredEdges.length}</strong></div><div><small>資料來源 Source</small><strong>runes.json</strong></div></div><div className="loc-grid two"><div><h3>節點 Nodes</h3><div className="loc-context-list">{filteredNodes.slice(0,40).map(node=><article className="loc-context-item compact" key={node.id}><b>{node.name}</b><span>{node.group}</span><small>{node.keywords.join(' · ')}</small></article>)}</div></div><div><h3>關係 Edges</h3><div className="loc-context-list">{filteredEdges.map((edge,index)=><article className="loc-context-item compact" key={`${edge.source}-${edge.target}-${edge.label}-${index}`}><b>{edge.source} ↔ {edge.target}</b><small>{edge.label}</small></article>)}</div></div></div></>}</section>}
  </section>;
}
