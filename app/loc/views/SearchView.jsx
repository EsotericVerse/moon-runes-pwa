...
function genericResult(item,source,terms,displayQuery){const hay=JSON.stringify(item);const matched=firstGovernedMatch(hay,terms);if(!matched)return null;const title=item.title||item.name||item['符文名稱']||item['名稱']||item.question||item.label||item.id||item.work_id||source;const body=item.text||item.content||item.answer||item.public_summary||item.summary||item.description||item.retrieval_text||hay;return {key:`${source}-${title}-${body.slice(0,30)}`,source,title,date:item.date||item.created_date||item.updated_at||'',snippet:snippet(body,matched||displayQuery),href:item.url||item.href||''};}
