...
function projectKnowledgeAssetsCurrent(value){if(Array.isArray(value))return value.map(projectKnowledgeAssetsCurrent);if(!value||typeof value!=='object')return value;const next={};for(const [key,item] of Object.entries(value)){if(/^historical|^legacy|^former|^superseded/i.test(key))continue;if(HISTORICAL_LOC_RE.test(key))continue;if(key==='related_scopes'&&Array.isArray(item)){next[key]=item.filter(scope=>!HISTORICAL_LOC_RE.test(String(scope)));continue;}if((key==='primary_loc'||key==='scope'||key==='owner')&&HISTORICAL_LOC_RE.test(String(item)))continue;next[key]=projectKnowledgeAssetsCurrent(item);}return next;}
function genericResult(item,source,terms,displayQuery){const hay=JSON.stringify(item);const matched=firstGovernedMatch(hay,terms);if(!matched)return null;const title=item.title||item.name||item['符文名稱']||item['名稱']||item.question||item.label||item.id||item.work_id||source;const body=item.text||item.content||item.answer||item.public_summary||item.summary||item.description||item.retrieval_text||hay;return {key:`${source}-${title}-${body.slice(0,30)}`,source,title,date:item.date||item.created_date||item.updated_at||'',snippet:snippet(body,matched||displayQuery),match:matched||displayQuery,href:item.url||item.href||''};}
function collectText(data,terms,displayQuery,found){for(const d of data?.documents||[]){const hay=`${d.title||''} ${d.section||''} ${d.retrieval_text||d.text||''}`;const matched=firstGovernedMatch(hay,terms);if(matched)found.push({key:d.id,source:'文字創作',title:[d.title,d.section].filter(Boolean).join(' · '),date:d.date||'',snippet:snippet(d.text||d.retrieval_text,matched||displayQuery),match:matched||displayQuery});if(found.length>=MAX_RAW_RESULTS)return;}}
function collectMusic(data,terms,displayQuery,found){for(const w of data?.works||[]){const hay=`${w.title||''} ${w.summary||''} ${w.style||''} ${(w.tags||[]).join(' ')} ${w.retrieval_text||''}`;const matched=firstGovernedMatch(hay,terms);if(matched)found.push({key:w.work_id,source:'音樂',title:w.title,date:w.created_date||'',snippet:snippet(w.summary||w.retrieval_text,matched||displayQuery),match:matched||displayQuery,href:w.versions?.[0]?.suno_url||''});if(found.length>=MAX_RAW_RESULTS)return;}}
...
export default function SearchView({fixedCollectionId=''}){
  ...
  async function runSearch(event){
    ...
    try{
      ...
      if(smallRequests.length){
        ...
        if(id!==searchId.current)return;
        for(let index=0;index<smallRequests.length;index+=1){const request=smallRequests[index];let data=smallData[index];if(request.label==='FAQ')data=applyFaqCurrentOverlay(data,faqOverlay);if(request.label==='知識庫')data=projectKnowledgeAssetsCurrent(data);collectGeneric(data,request.label,searchTerms,q,found);if(found.length>=MAX_RAW_RESULTS)break;}
      }
  ...
  return <section className="loc-view"><header className="loc-hero"><p className="loc-eyebrow">Search · 搜尋</p><h1>搜尋</h1><p>{collection.description} 大型資料清單（manifest）與資料分片（corpus shards）只有送出查詢後才下載。</p></header>
    ...
    <div className="loc-search-results">{shownResults.map(r=><article className="loc-card" key={r.key}><div className="loc-result-meta"><span>{r.source}</span>{r.date&&<time>{r.date}</time>}</div><h2><HighlightText text={r.title} term={r.match||query}/></h2><p><HighlightText text={r.snippet} term={r.match||query}/></p>{r.href&&<a href={r.href} target={/^https?:/.test(r.href)?'_blank':undefined} rel={/^https?:/.test(r.href)?'noreferrer':undefined}>查看來源</a>}</article>)}</div>
