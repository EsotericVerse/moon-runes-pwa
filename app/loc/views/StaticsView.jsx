...
export default function StaticsView(){
  ...
  useEffect(()=>{setDailyPage(1);setSourcePage(1)},[pageSize]);
  ...
  return <section className="loc-view">
    ...
    {tab==='daily'&&<div className="loc-grid two">
      ...
      <section className="loc-card"><p className="loc-eyebrow">Daily Rune History</p><h2>每日符文趨勢</h2>{daily?<><div className="loc-metrics"><div><small>紀錄數</small><strong>{draws.length}</strong></div><div><small>主抽</small><strong>{primary}</strong></div><div><small>補抽</small><strong>{draws.length-primary}</strong></div></div><div className="loc-history">{draws.slice((dailyPage-1)*pageSize,dailyPage*pageSize).map(x=><div key={x.id}><time>{x.date}</time><b>{x.rune}</b><span>{x.direction}</span><small>{x.draw_kind==='daily_draw'?'主抽':'補抽'}</small></div>)}</div>{draws.length>pageSize&&<div className="loc-pagination"><span>第 {dailyPage} / {dailyPages} 頁 · 每頁 {pageSize}</span><div><button className="loc-button" disabled={dailyPage<=1} onClick={()=>setDailyPage(p=>p-1)}>上一頁</button><button className="loc-button" disabled={dailyPage>=dailyPages} onClick={()=>setDailyPage(p=>p+1)}>下一頁</button></div></div>}</>:<p>載入中…</p>}</section>
    </div>}
