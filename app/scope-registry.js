...
export function scopeRoute(scopeOrConfig,name){const config=typeof scopeOrConfig==='string'?getScope(scopeOrConfig):scopeOrConfig;return `${config.canonicalOrigin}/${name}`.replace(/([^:]\/)\/+/g,'$1');}
export function equivalentPathRoute(scopeOrConfig,name=''){const config=typeof scopeOrConfig==='string'?getScope(scopeOrConfig):scopeOrConfig;return `${config.pathBase}/${name}`.replace(/\/+/g,'/')||'/';}
