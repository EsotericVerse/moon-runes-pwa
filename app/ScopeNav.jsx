...
export default function ScopeNav(){
  ...
  function submitSearch(event){
    ...
    window.location.assign(navRoute(cfg,'search'));
  }
  return <>
    <a href={cfg.reserved[1]}>{cfg.reserved[0]}</a>
    {SHARED_NAV_FUNCTIONS.map(([label,name])=><a key={name} href={navRoute(cfg,name)}>{label}</a>)}
    <form className="loc-next-search" onSubmit={submitSearch} role="search"><input name="q" type="search" aria-label="搜尋文字" placeholder="搜尋"/><button type="submit">搜尋</button></form>
