...
export default function ScopeManagementV2(){
  ...
  useEffect(()=>{
    ...
    if(account.user&&account.canManage)reload();
  ...
  async function moveTree({dragIds,parentId}){const child=dragIds?.[0];if(!child||!parentId)return;try{await requestScopeRelation({parent:parentId,child},{rows:scopeRelationsToNodes(relations),requestedBy:account.user?.id});await reload();}catch(e){setError(e?.message||'Scope 關係申請失敗');}}
  async function decide(item,status){try{await decideScopeRelationRequest(item.id,status,'',account.user?.id);await reload();}catch(e){setError(e?.message||'申請處理失敗');}}
