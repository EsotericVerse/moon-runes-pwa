...
export default function SearchV2(){
  ...
  useEffect(()=>{let live=true;setLoading(true);setError('');if(!view){setRows([]);setError('此 Scope 尚未設定 Neon Culture projection。');setLoading(false);return()=>{live=false};}neonClient.from(view).select('*').order('date',{ascending:true}).limit(5000).then(result=>{if(result?.error)throw new Error(result.error.message||'Search Neon projection 讀取失敗');if(live)setRows((result?.data||[]).map(normalize));}).catch(errorValue=>live&&setError(String(errorValue?.message||errorValue))).finally(()=>live&&setLoading(false));return()=>{live=false};},[view]);
  ...
  useEffect(()=>{setAnchor(matches[0]||null);setSelected(null);setStatus(matches.length?'找到 '+matches.length+' 項與「'+submitted+'」相關的內容。':'尚未找到「'+submitted+'」的時間錨點。');},[matches,submitted]);
