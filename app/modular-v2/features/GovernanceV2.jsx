...
const PROFILES=Object.freeze({
  loc:Object.freeze({
    ...
    cards:Object.freeze([
      ...
      Object.freeze({eyebrow:'法律',title:'Scope 邊界與資料責任',text:'各 Scope 保有自己的名稱、資料所有權、作者身分與公開邊界；共享功能只能讀取授權 canonical data。'}),
  ...
  runes:Object.freeze({
    ...
    cards:Object.freeze([
      ...
      Object.freeze({eyebrow:'Canonical Data',title:'Read Canonical Data',text:'公開頁面讀取 LunaRunes 自己的 canonical data；共享功能不改寫其 Master Data。'})
    ])
  }),
  lo3rwang:Object.freeze({
    ...
    cards:Object.freeze([
      ...
      Object.freeze({eyebrow:'Boundary',title:'個人資料邊界',text:'共享搜尋與脈絡功能只讀授權 canonical data，不因命中作品而擴張到未公開資料。'})
    ])
  }),
  admin:Object.freeze({
...
function genericProfile(scope){
  return Object.freeze({
    subtitle:`${scope.label} 的治理入口：Scope 邊界、資料授權資料、共用 Feature 與 Page Composition。`,
    cards:Object.freeze([
      Object.freeze({
        eyebrow:'Boundary',
        title:'Scope 邊界',
        text:'此 Scope 使用共用治理框架；名稱、資料、角色與公開授權資料依自身 Registry 設定，不自動繼承其他 Scope 的身份或資料所有權。'
      }),
      Object.freeze({
        eyebrow:'Composition',
        title:'Page Composition',
        text:'共用 Feature 使用同一 renderer；Scope 差異由 Registry 與資料 canonical data 注入。'
      })
    ])
  });
}
...
const GOVERNANCE_BOXES=Object.freeze({
  loc:Object.freeze([
    ...
    Object.freeze({eyebrow:'Scope · Data',title:'資料與責任',text:'每個 Scope 管理自己的資料、身份與公開邊界；共用功能只讀授權 canonical data，不因功能整合而混合資料權威。'}),
  ...
  default:Object.freeze([
    ...
    Object.freeze({eyebrow:'Scope · Data',title:'資料與責任',text:'資料、身份與公開邊界由各自 Scope 管理；共用功能只讀授權 canonical data。'}),
...
export default function GovernanceV2({section=null}){
  ...
  useEffect(()=>{
    ...
    selectScopeCultureData(scopeId).then(value=>{if(live)setHistory(value?.runeHistory||{});}).catch(()=>{if(live)setHistory(null);});
    return()=>{live=false};
  },[scopeId]);
