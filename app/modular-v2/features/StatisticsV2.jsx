...
export default function StatisticsV2({section=null}){
  ...
  return <FeaturePageV2 featureId="statics" subtitle="從時間長河計算單位、密度、來源與排行榜，回饋下一輪脈絡整理。">
    <ScopeCardV2 eyebrow="Statistics · Time River" title="時間長河統計">
      ...
      {mode==='keywords'?<div className="statistics-ranking"><div className="scope-v2-tabs" aria-label="排行榜類型">{rankingTypes.map(item=><button type="button" key={item} aria-pressed={rankingType===item} onClick={()=>{setRankingType(item);setPage(1)}}>{item}</button>)}</div><div className="statistics-top-ten"><p className="scope-v2-eyebrow">TOP 10 KEYWORDS · TIME ANCHOR</p><p>點選關鍵字，先看它在時間長河前後的分佈，再決定是否建立或調整時期。</p><div className="scope-v2-ranking">{topTen.map((row,index)=>{const term=row.term||row.title||row.name||'—';return <div key={'top-'+(row.ranking_key||term||index)}><b>{index+1}. {term}</b><span>{row.item_count??'—'} · <a href={'/search?q='+encodeURIComponent(term)}>查前後分佈</a></span></div>})}</div></div><div className="scope-v2-ranking">{shown.map((row,index)=><div key={row.ranking_key||row.term||index}><b>{(page-1)*PAGE_SIZE+index+1}. {row.term||row.title||row.name||'—'}</b><span>{row.item_count??'—'} · {row.rank_value??'—'} · <a href={'/search?q='+encodeURIComponent(row.term||row.title||row.name||'')}>查時間分佈</a></span></div>)}</div>{!loading&&!shown.length?<p>目前沒有可顯示的排行榜資料。</p>:null}{rankings.length?<div className="scope-v2-pagination"><span>第 {page} / {pages} 頁 · 共 {rankings.length} 筆</span><div><button type="button" disabled={page<=1} onClick={()=>setPage(value=>Math.max(1,value-1))}>上一頁</button><button type="button" disabled={page>=pages} onClick={()=>setPage(value=>Math.min(pages,value+1))}>下一頁</button></div></div>:null}</div>:null}
    </ScopeCardV2>
