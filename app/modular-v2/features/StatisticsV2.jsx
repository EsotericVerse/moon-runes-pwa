...
export default function StatisticsV2({section=null}){
  ...
  const topTen=rankings.slice(0,rankLimit);
  const termHistory=useMemo(()=>{const map=new Map();for(const row of rankingRows){const term=row.term||row.title||row.name;if(!term)continue;const current=map.get(term)||new Set();current.add(row.ranking_type||'總榜');map.set(term,current);}return map;},[rankingRows]);
  ...
  return <FeaturePageV2 featureId="statics" subtitle="從時間長河計算單位、密度、來源與排行榜，回饋下一輪脈絡整理。">
    <ScopeCardV2 eyebrow="Statistics · Time River" title="時間長河統計">
      ...
      {mode==='keywords'?<div className="statistics-ranking"><div className="scope-v2-tabs" aria-label="排行榜類型">{rankingTypes.map(item=><button type="button" key={item} aria-pressed={rankingType===item} onClick={()=>{setRankingType(item);setPage(1)}}>{item}</button>)}</div><div className="statistics-top-ten"><p className="scope-v2-eyebrow">TOP KEYWORDS · TIME ANCHOR</p><p>指定前幾名後勾選要一起分析的關鍵詞；系統只整理分佈與重複排名，時期仍由你判斷。</p><label>顯示前幾名<select value={rankLimit} onChange={event=>setRankLimit(Number(event.target.value))}><option value="5">前 5 名</option><option value="10">前 10 名</option><option value="20">前 20 名</option></select></label><div className="scope-v2-ranking">{topTen.map((row,index)=>{const term=row.term||row.title||row.name||'—';const repeated=termHistory.get(term)?.size||1;return <div key={'top-'+(row.ranking_key||term||index)}><label><input type="checkbox" checked={selectedTerms.includes(term)} onChange={()=>setSelectedTerms(current=>current.includes(term)?current.filter(value=>value!==term):[...current,term])}/><b>{index+1}. {term}</b></label><span>{row.item_count??'—'} · {repeated>1?'跨 '+repeated+' 個排名':'單一排名'} · <a href={'/search?q='+encodeURIComponent(term)}>單詞分佈</a></span></div>})}</div><p><a className="context-btn primary" href={batchHref}>批次查看選取關鍵詞</a></p></div><div className="scope-v2-ranking">{shown.map((row,index)=><div key={row.ranking_key||row.term||index}><b>{(page-1)*PAGE_SIZE+index+1}. {row.term||row.title||row.name||'—'}</b><span>{row.item_count??'—'} · {row.rank_value??'—'} · <a href={'/search?q='+encodeURIComponent(row.term||row.title||row.name||'')}>查時間分佈</a></span></div>)}</div>{!loading&&!shown.length?<p>目前沒有可顯示的排行榜資料。</p>:null}{rankings.length?<div className="scope-v2-pagination"><span>第 {page} / {pages} 頁 · 共 {rankings.length} 筆</span><div><button type="button" disabled={page<=1} onClick={()=>setPage(value=>Math.max(1,value-1))}>上一頁</button><button type="button" disabled={page>=pages} onClick={()=>setPage(value=>Math.min(pages,value+1))}>下一頁</button></div></div>:null}</div>:null}
    </ScopeCardV2>
