(() => {
  ...
  function paintNav(node, items = DEFAULT_NAV) {
    ...
    node.innerHTML = `
      ...
      <div class="loc-global-links">
        ${items.filter(item => item.id !== "search").map(item => `
          <a class="loc-global-link" href="${esc(item.href)}"${item.id === active ? ' aria-current="page"' : ""}>${esc(item.label)}</a>
        `).join("")}
  ...
  function buildRunesTiers(host) {
    ...
    const renderThird = mode => {
      ...
      if (mode === "library") {
        GROUPS.forEach(group => inner.appendChild(link(group, `runes.html?group=${encodeURIComponent(group)}#library`)));
      } else if (mode === "draw") {
        [
    ...
    t2.addEventListener("click", event => {
      ...
      if (!control) return;
      const view = control.dataset.runesView;
      renderThird(view === "library" ? "library" : view === "draw" ? "draw" : "");
    });
  }
  ...
  function buildEvolutionTier(host) {
    const controls = [
      ["時期", '[data-view="overview"]'],
      ["時間線", '[data-view="timeline"]'],
      ["趨勢", '[data-view="trend"]'],
      ["軌跡", '[data-view="trajectory"]']
    ].map(([labelText, selector], index) => relabelControl(firstControl([selector], index), labelText) || link(labelText, `evolution.html#${labelText}`));
  ...
  function removeLegacySidebars() {
    document.querySelectorAll(".workspace-sidebar,.sidebar").forEach(sidebar => sidebar.remove());
  }
