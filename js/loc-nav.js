(()=>{
  ...
  function scope(){
    ...
    if(host==='lrunes.lo3rwang.cc') return 'runes';
    if(host==='lo3rwang.lo3rwang.cc'||host==='lo3rwang.cc') return 'lo3rwang';
    if(host==='admin.lo3rwang.cc') return 'admin';
    if(host==='loc.lo3rwang.cc') return 'loc';
    if(path==='/admin'||path.startsWith('/admin/')||path==='/management'||path.startsWith('/management/')) return 'admin';
  ...
  function navItems(){
    ...
    if(current==='runes') return [
      {label:'語彙',href:'https://lrunes.lo3rwang.cc/list'},
      {label:'脈絡',href:'/context'},{label:'統計',href:'/statics'},{label:'文化',href:'/evolution'},{label:'治理',href:'/governance'},
      {label:'管理者首頁',href:'https://lo3rwang.lo3rwang.cc'},
    ...
    return [
      {label:'月之符文',href:'https://lrunes.lo3rwang.cc'},{label:'脈絡',href:'/context'},{label:'統計',href:'/statics'},{label:'文化',href:'/evolution'},{label:'治理',href:'/governance'},
      {label:'管理者首頁',href:'https://lo3rwang.lo3rwang.cc'},{label:'回月典首頁',href:'https://loc.lo3rwang.cc',home:true}
    ];
  }
  function renderNav(){
    const items=navItems();
    document.querySelectorAll('[data-loc-nav]').forEach(node=>{
      const searchAction=scope()==='admin'?null:scopedRoute('/search');
      ...
      const search=searchAction?`<form class="loc-global-search" action="${esc(searchAction)}" method="get" role="search"><input name="q" type="search" aria-label="搜尋文字" placeholder="輸入文字" /><button class="loc-global-search-submit" type="submit">搜尋</button></form>`:'';
  ...
  function buildRunes(host){
    ...
    if(drawView&&!drawView.querySelector(':scope > .loc-quick-menu')){
      const menu=quickMenu('runes:draw',[
        link('單卡','?mode=single'),link('每日','?mode=daily'),link('雙卡','?mode=2card'),link('三卡','?mode=3card'),link('五卡','?mode=5card'),link('11卡 OW3gs','?mode=ow3gs')
      ],'抽牌快速選單');
      if(menu)drawView.appendChild(menu);
    }
    const libraryView=document.getElementById('libraryView');
    if(libraryView&&!libraryView.querySelector(':scope > .loc-quick-menu')){
      const menu=quickMenu('runes:library',GROUPS.map(group=>link(group,`?view=library&group=${encodeURIComponent(group)}`,{'data-rune-group-shortcut':group})),'符文群組快速選單');
