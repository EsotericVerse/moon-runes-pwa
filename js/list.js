  ...
  function organizeRuneWorkspace(){
    ...
    if (referenceView) {
      ...
      [...referenceView.querySelectorAll('.basic-item')].forEach(item => {
        ...
        if (strong?.textContent.trim() === '不用背卡' && span) span.textContent = '先抽、先看，再需要時展開符文本義與脈絡。';
      });
    ...
    if (drawView && !document.getElementById('draw-algorithm-intro')) {
      ...
      if (selector) selector.after(section); else drawView.prepend(section);
    }
    ...
    if (systemsView) {
      ...
      [...systemsView.querySelectorAll('.reading-ref-card')].forEach(card => {
        ...
        if (!heading) return;
        if (heading.textContent.trim() === '符文體系') card.innerHTML = `<h3>符文體系</h3><p>符文可以延伸成不同形式的作品與表達方式；作品保留自己的內容，同時記錄它與符文來源之間的關係。</p><ul><li><strong>符文文學：</strong>把符文展開成角色、情境、事件、段落與完整故事。代表作品可從 <a href="https://vocus.cc/article/6829d52cfd8978000109b281" target="_blank" rel="noopener">《月語者》第一篇〈現在，下弦之卷〉</a> 開始閱讀。</li><li><strong>符文歌曲：</strong>以實際 OW3gs 11 張抽牌結果作為創作來源，再轉化成歌詞與音樂。現行確認作品包括〈在塵裡長出的光〉、〈日蝕之前的顯現〉、〈界內之風〉、〈界外誤差〉。<a href="search.html?q=%E7%AC%A6%E6%96%87%E6%AD%8C%E6%9B%B2">查看符文歌曲清單與作品連結 →</a></li></ul>`;
        if (heading.textContent.trim() === '符文演算') card.innerHTML = `<h3>符文演算法</h3><p>符文演算法講的是「方法」：把符文本義、方向、卡位與組合規則整理成可以重複使用的判讀流程。單卡、雙卡、三卡、五卡與 OW3gs 都是不同的抽牌演算法。</p><p>演算法負責怎麼算、怎麼讀；它不等於符文本身，也不等於符文體系作品。</p>`;
        if (heading.textContent.trim() === '符文演化') card.innerHTML = `<h3>符文演化</h3><p>符文演化講的是「時間中的改變」：觀察符文、作品與概念在不同時期如何出現、偏移、重組與形成趨勢。</p><p>演算法處理方法；演化處理時間與變化，兩者分開。</p>`;
      });
    }
  }
