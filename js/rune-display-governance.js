(() => {
  ...
  function makeDrawField(label,value,authority){
    ...
    if(authority) div.dataset.semanticAuthority=authority;
  ...
  function governDrawCard(card){
    const name=runeNameFromHeading(card.querySelector('.rune-result-name'));
  ...
  function governLibraryTile(tile){
    ...
    if(!spec && clean(rune.顯化形式)){
      ...
      }else spec=makeInfoBox('Spec／顯化形式',rune.顯化形式,'spec');
      host.prepend(spec);
    } else if(spec){ spec.dataset.semanticAuthority='spec'; }
  ...
  function governSearchOracle(card){
    ...
    const name=clean(identity.split('·')[0]);
    const rune=runeMap.get(name);
    if(!rune) return;
      ...
      new MutationObserver(records=>{
        for(const record of records){
          for(const node of record.addedNodes){
            if(node.nodeType===1) govern(node.matches?.('.rune-result-card,.rune-tile,.special-rune-card,article.card')?node.parentElement||node:node);
          }
        }
