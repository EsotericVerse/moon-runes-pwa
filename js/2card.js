...
function buildLocalDivination(cards, realPhase) {
  ...
  const phaseLine = cards.map(card => `${card.label}為${card.rune.月相}卡`).join('，');
  const explanation = cards.map(card => {
    const base = `${card.label}「${card.rune.符文名稱}」屬於${card.rune.所屬分組}，以${card.direction}呈現。`;
    const task = card.rune.靈魂課題 || card.rune.實踐挑戰 || card.rune.分組說明 || '此牌提醒你回到當下，觀察事情真正的流向。';
    return `${base}${task}`;
  }).join('<br>');

  return `
    <p><strong>完整現況：</strong>${phaseLine}，目前真實月相為${realPhase}。這組牌面顯示，事情正在由「因」推向「果」，重點是看清前後關係，而不是急著下結論。</p>
    <p><strong>牌面解說：</strong>${explanation}</p>
    <p><strong>占卜結論：</strong>${names}。先確認造成現況的核心原因，再處理它導向的結果；能修正的先修正，不能控制的先放回時間。</p>
    <hr>
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const fallbackHtml = buildLocalDivination([
    { label: '因', rune: rune1, direction: direction1 },
    { label: '果', rune: rune2, direction: direction2 }
  ], realPhase);
