(() => {
  ...
  function addGovernanceNote(){
    ...
    box.innerHTML = `
      ...
      <p><strong>LOC 是語言系統模組框架：以中立方式切分語言，把模糊問題拆成可處理的模組、方法與演算法，不替使用者決定哪個答案必然正確。</strong></p>
  ...
  function addRunesNote(){
    ...
    box.innerHTML = `
      ...
      <p><strong>LOC 是語言系統模組框架，提供中立的切分、模組化、方法與演算法。</strong> 月之符文提供一套已實際運作的符號式語言模組作為參考實證，LOC 再把這種切分、組合、脈絡與推演方法擴展到其他語言系統，因此兩者相輔相成。</p>`;
    host.after(box);
  }
  ...
  function addEvolutionNote(){
    ...
    box.innerHTML = `
      ...
      <p>LOC 這個語言系統模組框架把同樣的中立切分方式擴展成模組、方法與演算法：<strong>不確定性 → 語意拆分 → 模組化可能性 → 路徑比較 → 人的選擇 → 新結果 → 再次推演</strong>。選擇仍屬於人，新的結果再成為下一輪語言與脈絡的輸入。</p>`;
    hero.after(box);
  }
