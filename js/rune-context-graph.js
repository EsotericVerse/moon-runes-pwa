(()=>{
  ...
  function buildGraph(runes,derivedEntries=[],groupNames=[]){
    const nodes=new Map(),edges=new Map();
    const groups=groupNames.filter(Boolean),groupSet=new Set(groups),defaultGroup=groups.at(-1)||'';
    ...
    for(const rune of runes||[]){
      ...
      const rawGroup=String(rune?.所屬分組||'').trim(),group=groupSet.has(rawGroup)?rawGroup:defaultGroup;
      ...
      for(const rule of parseOwnershipRules(rune?.額外規則,runeNames)){
        const sid=nodeId('term',rule.source);addNode(nodes,{id:sid,label:rule.source,type:'term',group:defaultGroup});
        const targetId=rule.targetRune?nodeId('rune',rule.targetRune):nodeId('term',rule.target);
        addNode(nodes,{id:targetId,label:rule.target,type:rule.targetRune?'rune':'term',group:rule.targetRune?(runeGroup.get(rule.targetRune)||defaultGroup):defaultGroup});
    ...
    for(const entry of derivedEntries||[]){
      ...
      const relation=String(entry?.relation||'derived'),relationId=nodeId('relation_type',relation);addNode(nodes,{id:relationId,label:relation,type:'relation_type',group:defaultGroup});addEdge(edges,{source:tid,target:relationId,type:'uses_relation'});
      const status=String(entry?.status||'special'),statusId=nodeId('status',status);addNode(nodes,{id:statusId,label:status,type:'status',group:defaultGroup});addEdge(edges,{source:tid,target:statusId,type:'has_status'});
      ...
      else if(defaultGroup)addEdge(edges,{source:tid,target:nodeId('group',defaultGroup),type:'remains_special'});
    }

    return {version:'shared-static-runtime-v1',api_used:false,default_group:defaultGroup,groups,derived_count:(derivedEntries||[]).length,nodes:[...nodes.values()],edges:[...edges.values()]};
  }
  ...
  function resolveTerm(graph,query){
    ...
    return {term,status:node.term_status||'canonical',relation:node.relation||chosen?.type||'canonical',resolved_rune:rune,group:runeNode?.group||node.group||graph.default_group};
  }
  ...
  function mount(graph){
    ...
    groupSel.innerHTML='<option value="">全部群組</option>'+graph.groups.map(g=>`<option value="${esc(g)}">${esc(g)}</option>`).join('');
  ...
  async function init(){
    ...
    try{
      ...
      const graph=buildGraph(data.rows,data.derivedEntries,[...data.groups.keys()]);window.LunaRuneSemanticGraph=graph;mount(graph);
