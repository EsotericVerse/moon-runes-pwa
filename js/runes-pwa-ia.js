(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .beginner-slide:before{content:"☾";position:absolute;left:4%;top:3%;font-size:clamp(2.4rem,6vw,6rem);color:var(--gold);opacity:.95}
      .beginner-slide:after{content:"✦  ✦  ✦";position:absolute;right:4%;top:5%;color:var(--gold);letter-spacing:.65rem;font-size:clamp(.65rem,1.3vw,1.1rem);opacity:.9}
      .beginner-inner{position:absolute;inset:0;padding:7% 8%;display:grid;align-content:center;gap:18px}
      .beginner-num{position:absolute;left:2.7%;bottom:2.8%;z-index:2;color:#74c7ff;font-size:.72rem;font-weight:900;letter-spacing:.05em}
      .beginner-title-xl,.beginner-title-lg,.beginner-title-md{margin:0;text-align:center;letter-spacing:-.04em}
      .beginner-title-xl{font-size:clamp(2.1rem,5vw,5rem);line-height:1}
      .beginner-title-lg{font-size:clamp(1.6rem,3.5vw,3.4rem);line-height:1.08}
      .beginner-title-md{font-size:clamp(1.25rem,2.8vw,2.6rem);line-height:1.12}
      .beginner-sub{margin:0 auto;color:var(--muted);font-size:clamp(.78rem,1.35vw,1.2rem);max-width:90%;text-align:center}
      .beginner-glass{width:min(92%,1080px);margin:auto;padding:4.2% 4.8%;border:1px solid rgba(177,217,255,.25);border-radius:22px;background:rgba(245,250,255,.9);color:#254f72;box-shadow:0 0 0 10px rgba(38,93,143,.18)}
      ...
      @media(max-width:720px){
        .runes-third-nav{margin-bottom:10px;padding:6px;gap:5px;border-radius:11px}
        .runes-third-link{min-height:44px;padding:8px 10px;font-size:.76rem}
        .runes-third-nav-label{display:none}
        .beginner-slide{min-height:560px;border-radius:15px}
        .beginner-inner{position:relative;inset:auto;min-height:560px;padding:68px 18px 34px}
        .beginner-glass{width:100%;padding:22px 18px}.beginner-glass p,.beginner-glass ul{font-size:.92rem}.beginner-two{grid-template-columns:1fr}.beginner-overview{width:100%}.beginner-title-xl{font-size:2.3rem}.beginner-title-lg{font-size:1.85rem}.beginner-title-md{font-size:1.45rem}
      }
    `;
  ...
  async function ensureBeginnerView() {
    ...
    view.hidden = true;
    view.innerHTML = `
      <header class="hero loc-header">
        <p class="loc-header-meta">LunaRunes · Beginner Guide</p>
        <h1 class="loc-header-title">新手入門</h1>
        <h2 class="loc-header-subtitle">月之符文入門直接整合在 Lots。</h2>
      </header>
      <div class="runes-beginner-actions">
        <a class="runes-beginner-action primary" href="runes.html?mode=daily#draw">先抽一張</a>
        <a class="runes-beginner-action" href="runes.html#library">月之符文66 圖鑑</a>
        <a class="runes-beginner-action" href="docs/LOC_Tutorial_02_月之符文入門.pdf" target="_blank" rel="noopener">PDF 教材</a>
      </div>
      <div id="runesBeginnerContent"><div class="empty">載入新手入門…</div></div>`;
    main.prepend(view);
  ...
  function setView(name) {
    ...
    document.querySelectorAll(".runes-view").forEach(view => {
      if (view === beginner) view.hidden = name !== "beginner";
      else if (name === "beginner") view.hidden = true;
    });
    document.querySelectorAll("[data-runes-view]").forEach(button => button.classList.toggle("active", button.dataset.runesView === name));
    if (name === "beginner") history.replaceState(null, "", "runes.html#beginner");
  }

  function wireBeginnerNavigation() {
    document.addEventListener("click", event => {
      const button = event.target.closest('[data-runes-view="beginner"]');
      if (button) {
    ...
    document.querySelectorAll('a[href="tutorial02.html"],a[href$="/tutorial02.html"]').forEach(anchor => {
      anchor.href = "runes.html#beginner";
      anchor.removeAttribute("target");
      anchor.removeAttribute("rel");
    });

    if (location.hash === "#beginner") setTimeout(() => setView("beginner"), 0);
  }
  ...
  function addDrawThirdLevel() {
    ...
    if (!drawView || !selector || document.getElementById("drawThirdNav")) return;
    const nav = document.createElement("nav");
    nav.id = "drawThirdNav";
    nav.className = "runes-third-nav";
    nav.setAttribute("aria-label", "抽牌數量快速選單");
    const mode = new URLSearchParams(location.search).get("mode") || "";
    nav.innerHTML = '<span class="runes-third-nav-label">LOTS</span>' + DRAW_MODES.map(([value, label]) => `<a class="runes-third-link${mode === value ? " active" : ""}" href="runes.html?mode=${value}#draw">${label}</a>`).join("") + '<span class="runes-third-link" aria-disabled="true" title="目前頁面尚未啟用 OW3gs 執行">OW3gs 11</span>';
    selector.before(nav);
  }
  ...
  function addLibraryThirdLevel() {
    ...
    const nav = document.createElement("nav");
    nav.id = "libraryThirdNav";
    nav.className = "runes-third-nav";
    nav.setAttribute("aria-label", "符文群組快速選單");
  ...
  function bindExistingQuickSelector() {
    document.addEventListener("click", event => {
  ...
  async function init() {
    ...
    addStyles();
    normalizeWorkspaceNav();
    await ensureBeginnerView();
    normalizeWorkspaceNav();
    wireBeginnerNavigation();
    addDrawThirdLevel();
    addLibraryThirdLevel();
    bindExistingQuickSelector();
    setTimeout(normalizeWorkspaceNav, 250);
  }
