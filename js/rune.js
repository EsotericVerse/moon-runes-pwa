...
async function initRunePage() {
  ...
  const runeRows = runtimeRunes.filter(Boolean);
  const groups = Array.isArray(runtimeGroups) ? runtimeGroups : [];
  ...
  function normalize(row){
    ...
    const meta = groupByRuneId.get(id) || row.group_meta || null;
  ...
  if (overviewLink && coreGroups.length && !document.getElementById('rune-group-quick-selector')) {
...
(() => {
  ...
  function realMoonPhase(){return phaseCategory(window.LOCMoonPhase?.getRealPhase?.() || sessionStorage.getItem('realPhase') || '未知');}
  function valueOf(rune,...keys){for(const key of keys){const value=clean(rune?.[key]);if(value)return value;}return '—';}
  function runeId(rune){const id=Number(rune?.編號 ?? rune?.id);return Number.isInteger(id)?id:null;}
  function cardAttribute(rune){const raw=valueOf(rune,'卡片屬性','card_attribute');if(['正面','正向'].includes(raw))return '正面';if(['負面','負向'].includes(raw))return '負面';if(['中平','中性','中立'].includes(raw))return '中立';return '未知';}
  function cardPhase(rune){const raw=valueOf(rune,'月相','moon_phase');if(raw==='無')return '無';if(PHASES.has(raw))return raw;return raw||'未知';}
  function groupLabel(rune){const meta=groupByRuneId.get(runeId(rune));const zh=clean(meta?.group_zh)||valueOf(rune,'所屬分組','group');const en=clean(meta?.group_en);return en?`${zh} ${en}`:zh;}
  ...
  function runeNameFromHeading(node){if(!node)return '';const explicit=clean(node.dataset?.runeName);if(explicit)return explicit;const clone=node.cloneNode(true);clone.querySelectorAll('small,span').forEach(el=>el.remove());return clean(clone.textContent).replace(/^符文名稱\s*[：:]\s*/,'').replace(/\s*\([^)]*\)\s*$/,'').replace(/之符文$/,'');}
  function titleText(rune){const name=valueOf(rune,'符文名稱','名稱','name');const english=valueOf(rune,'英文','english');return `符文名稱：${name}之符文 (${english})`;}
  function primaryFields(rune){const note=valueOf(rune,'符文說明','特別說明','說明','description');const archetype=valueOf(rune,'人格原型','archetype');return [['說明',`${note} / ${archetype}`],['所屬分組',groupLabel(rune)]];}
  function detailFields(rune){const keyword=valueOf(rune,'正向關鍵詞','關鍵詞','keyword');const reverse=valueOf(rune,'反向關鍵詞','反向關鍵字','reverse_keyword');return [['關鍵詞',`${keyword} / ${reverse}`],['卡片詞性',cardAttribute(rune)],['月相',`卡片 ${cardPhase(rune)} / 目前 ${realMoonPhase()}`]];}
  function fieldHtml(label,value,type=''){const emphasis=type==='position'?' loc-rune-position-bubble':'';return `<div class="rune-result-field loc-rune-info-bubble${emphasis}"><span class="rune-result-label">${esc(label)}：</span><span class="rune-result-value">${esc(value)}</span></div>`;}
  function detailsHtml(rune){return `<details class="loc-rune-details"><summary>關鍵詞 · 卡片詞性 · 月相</summary><div class="loc-rune-details-body">${detailFields(rune).map(([label,value])=>fieldHtml(label,value)).join('')}</div></details>`;}
  function infoGridHtml(rune,direction=''){const primary=primaryFields(rune).map(([label,value])=>`<div class="loc-rune-info-bubble"><strong>${esc(label)}：</strong>${esc(value)}</div>`).join('');const position=clean(direction)?`<div class="loc-rune-info-bubble loc-rune-position-bubble"><strong>卡片位向：</strong>${esc(clean(direction))}</div>`:'';return `<div class="loc-rune-info-grid">${primary}${detailsHtml(rune)}${position}</div>`;}
  function titleBubbleHtml(rune){return `<div class="loc-rune-title-bubble" data-rune-name="${esc(valueOf(rune,'符文名稱','名稱','name'))}">${esc(titleText(rune))}</div>`;}
  ...
  function governDrawCard(card){
    ...
    const directionField=originalFields.find(field=>{const label=clean(field.querySelector('.rune-result-label')?.textContent).replace(/[：:]$/,'');return label==='卡片方向'||label==='卡片位向';});
  ...
  function governLibraryTile(tile){
    ...
    if(!rune)return;
    if(tile.matches('.special-rune-card')){const copy=tile.querySelector('.special-rune-copy');if(!copy)return;copy.innerHTML=`${titleBubbleHtml(rune)}${infoGridHtml(rune)}`;}
    else{const info=tile.querySelector('.rune-info');if(!info)return;info.innerHTML=`${titleBubbleHtml(rune)}${infoGridHtml(rune)}`;}
    markGoverned(tile);
  }
  ...
  function governSearchOracle(card){
    ...
    [...card.querySelectorAll('p.summary')].forEach(p=>{const text=clean(p.textContent);if(text.startsWith('方位語意：'))p.dataset.semanticAuthority='direction';if(text.startsWith('延伸字樣：'))p.dataset.semanticAuthority='extension';});
    markGoverned(card);
  }
  ...
  function start(){
    ...
    runtimeGroups.forEach(group=>{(group.runes||[]).forEach(member=>{const id=Number(member.id);if(Number.isInteger(id))groupByRuneId.set(id,group);});});
    govern(document);
    new MutationObserver(records=>{for(const record of records){for(const node of record.addedNodes){if(node.nodeType===Node.ELEMENT_NODE)govern(node);}}}).observe(document.body,{childList:true,subtree:true});
  }
